...
public interface PostService {
    ...
    //게시글 목록 불러오기
    public PageResult<PostListItemDto> getPostsByPath(String categoryPath, String keyword,
                                     int pageNum, int pageSize, Long currentUserId);

    //마이페이지/ 타 유저 프로필에서 쓴 글 목록 가져오기
    //첫 번째 파라미터가 조회하려는 작성자ID/ 두번째 파라미터는 현재 로그인한 사용자ID
    PageResult<PostListItemDto> getPostSummariesByUserId(Long targetUserId, Long userId,int pageNum, int pageSize);

    // 댓글 단 글 목록 가져오기
    PageResult<PostListItemDto> getPostsCommentedByUser(Long targetUserId,Long userId,int pageNum, int pageSize);

    // 좋아요한 글 목록 가져오기
    PageResult<PostListItemDto> getPostsLikedByUser(Long targetUserId, Long userId, int pageNum, int pageSize);
}
